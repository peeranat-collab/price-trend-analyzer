...
# Top Driver Insight
    ...
    # Optional Buy/Sell Guidance
    st.subheader("💡 คำแนะนำเชิงจัดซื้อ")

    if total_impact_pct > 0:
        st.write(
            f"- ราคาควรซื้อปีนี้ **ไม่ควรเกิน {recommended_price:,.2f} บาท/หน่วย** "
            f"เนื่องจากต้นทุนมีแนวโน้มเพิ่มขึ้นโดยเฉลี่ย {total_impact_pct:.2f}%"
        )
    else:
        st.write(
            f"- ราคาควรซื้อปีนี้ **ประมาณ {recommended_price:,.2f} บาท/หน่วย** "
            f"เนื่องจากต้นทุนมีแนวโน้มลดลงหรือทรงตัว"
        )
